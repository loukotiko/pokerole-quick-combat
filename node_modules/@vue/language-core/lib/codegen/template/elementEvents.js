...
function* generateElementEvents(options, ctx, node, componentVar, componentInstanceVar, emitVar, eventsVar) {
    ...
    for (const prop of node.props) {
        if (prop.type === CompilerDOM.NodeTypes.DIRECTIVE
            ...
            const originalPropNameObjectKey = utils_1.variableNameRegex.test(originalPropName)
                ? originalPropName
                : `'${originalPropName}'`;
            ...
            yield `__VLS_IsFunction<typeof ${propsVar}, '${originalPropName}'> extends true${utils_1.newLine}`;
            yield `? typeof ${propsVar}${utils_1.newLine}`;
            yield `: __VLS_IsFunction<typeof ${eventsVar}, '${prop.arg.loc.source}'> extends true${utils_1.newLine}`;
            yield `? {${utils_1.newLine}`;
            yield `/**__VLS_emit,${emitVar},${prop.arg.loc.source}*/${utils_1.newLine}`;
            yield `${originalPropNameObjectKey}?: typeof ${eventsVar}['${prop.arg.loc.source}']${utils_1.newLine}`;
            yield `}${utils_1.newLine}`;
            if (prop.arg.loc.source !== (0, shared_1.camelize)(prop.arg.loc.source)) {
                yield `: __VLS_IsFunction<typeof ${eventsVar}, '${(0, shared_1.camelize)(prop.arg.loc.source)}'> extends true${utils_1.newLine}`;
                yield `? {${utils_1.newLine}`;
                yield `/**__VLS_emit,${emitVar},${(0, shared_1.camelize)(prop.arg.loc.source)}*/${utils_1.newLine}`;
                yield `${originalPropNameObjectKey}?: typeof ${eventsVar}['${(0, shared_1.camelize)(prop.arg.loc.source)}']${utils_1.newLine}`;
                yield `}${utils_1.newLine}`;
            }
            yield `: typeof ${propsVar}${utils_1.newLine}`;
            yield `) = {${utils_1.newLine}`;
...
function* generateEventArg(ctx, arg, enableHover) {
    const features = enableHover
        ...
        : eventArgFeatures;
    if (utils_1.variableNameRegex.test((0, shared_1.camelize)(arg.loc.source))) {
        yield ['', 'template', arg.loc.start.offset, features];
        yield `on`;
        yield* (0, camelized_1.generateCamelized)((0, shared_1.capitalize)(arg.loc.source), arg.loc.start.offset, utils_1.combineLastMapping);
    }
    else {
        yield* (0, utils_1.wrapWith)(arg.loc.start.offset, arg.loc.end.offset, features, `'`, ['', 'template', arg.loc.start.offset, utils_1.combineLastMapping], 'on', ...(0, camelized_1.generateCamelized)((0, shared_1.capitalize)(arg.loc.source), arg.loc.start.offset, utils_1.combineLastMapping), `'`);
    }
}
