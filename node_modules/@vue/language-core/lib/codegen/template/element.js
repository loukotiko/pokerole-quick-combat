...
function* generateComponent(options, ctx, node, currentComponent) {
    ...
    yield `(`;
    yield* (0, utils_1.wrapWith)(startTagOffset, startTagOffset + node.tag.length, ctx.codeFeatures.verification, `{`, ...(0, elementProps_1.generateElementProps)(options, ctx, node, props, options.vueCompilerOptions.strictTemplates, true, failedPropExps), `}`);
...
function* generateElement(options, ctx, node, currentComponent, componentCtxVar, isVForChild) {
    ...
    yield `)(`;
    yield* (0, utils_1.wrapWith)(startTagOffset, startTagOffset + node.tag.length, ctx.codeFeatures.verification, `{`, ...(0, elementProps_1.generateElementProps)(options, ctx, node, node.props, options.vueCompilerOptions.strictTemplates, true, failedPropExps), `}`);
    yield `)${utils_1.endOfLine}`;
    yield* generateFailedPropExps(options, ctx, failedPropExps);
...
function* generateComponentSlot(options, ctx, node, slotDir, currentComponent, componentCtxVar) {
    ...
    if (slotDir?.arg?.type === CompilerDOM.NodeTypes.SIMPLE_EXPRESSION && slotDir.arg.content) {
        ...
        yield `default: `;
        yield* (0, utils_1.wrapWith)(slotDir.loc.start.offset, slotDir.loc.start.offset + (slotDir.loc.source.startsWith('#')
            ? '#'.length
            : slotDir.loc.source.startsWith('v-slot:')
                ? 'v-slot:'.length
                : 0), ctx.codeFeatures.withoutHighlightAndCompletion, `__VLS_thisSlot`);
    }
    yield `} = __VLS_nonNullable(${componentCtxVar}.slots)${utils_1.endOfLine}`;
