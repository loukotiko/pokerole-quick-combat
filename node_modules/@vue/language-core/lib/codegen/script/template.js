...
function* generateTemplateComponents(options) {
    ...
    if (nameType) {
        exps.push(`{} as { [K in ${nameType}]: typeof __VLS_self & (new () => { `
    ...
    exps.push(`__VLS_ctx`);
    yield `const __VLS_localComponents = {${utils_1.newLine}`;
    ...
    yield `}${utils_1.endOfLine}`;
    yield `let __VLS_components!: typeof __VLS_localComponents & __VLS_GlobalComponents${utils_1.endOfLine}`;
}
function* generateTemplateDirectives(options) {
    ...
    exps.push(`__VLS_ctx`);
    yield `const __VLS_localDirectives = {${utils_1.newLine}`;
    for (const type of exps) {
        yield `...`;
        yield type;
        yield `,${utils_1.newLine}`;
    ...
    yield `}${utils_1.endOfLine}`;
    yield `let __VLS_directives!: typeof __VLS_localDirectives & __VLS_GlobalDirectives${utils_1.endOfLine}`;
}
...
function* generateTemplateBody(options, templateCodegenCtx) {
    ...
    yield `	slots: ${options.scriptSetupRanges?.slots.name ?? '__VLS_slots'},${utils_1.newLine}`;
