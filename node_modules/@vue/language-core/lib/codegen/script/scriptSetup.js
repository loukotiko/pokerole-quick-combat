...
function* generateScriptSetup(options, ctx, scriptSetup, scriptSetupRanges) {
    ...
    if (scriptSetup.generic) {
        ...
        yield `return {} as {${utils_1.newLine}`
            + `	props: ${ctx.localTypes.PrettifyLocal}<typeof __VLS_functionalComponentProps & __VLS_TemplateResult['attrs'] & __VLS_PublicProps> & __VLS_BuiltInPublicProps,${utils_1.newLine}`
            + `	expose(exposed: import('${options.vueCompilerOptions.lib}').ShallowUnwrapRef<${scriptSetupRanges.expose.define ? 'typeof __VLS_exposed' : '{}'}>): void,${utils_1.newLine}`
...
function* generateSetupFunction(options, ctx, scriptSetup, scriptSetupRanges, syntax, definePropMirrors) {
    if (options.vueCompilerOptions.target >= 3.3) {
        ...
        yield `} = await import('${options.vueCompilerOptions.lib}')${utils_1.endOfLine}`;
    }
    ...
    if (scriptSetupRanges.expose.define) {
        if (scriptSetupRanges.expose.define?.typeArg) {
            setupCodeModifies.push([
                [
                    `let __VLS_exposed!: `,
                    (0, utils_1.generateSfcBlockSection)(scriptSetup, scriptSetupRanges.expose.define.typeArg.start, scriptSetupRanges.expose.define.typeArg.end, index_1.codeFeatures.navigation),
                    `${utils_1.endOfLine}`,
                ],
            ...
            setupCodeModifies.push([
                [
                    `const __VLS_exposed = `,
                    (0, utils_1.generateSfcBlockSection)(scriptSetup, scriptSetupRanges.expose.define.arg.start, scriptSetupRanges.expose.define.arg.end, index_1.codeFeatures.navigation),
                    `${utils_1.endOfLine}`,
                ],
                scriptSetupRanges.expose.define.start,
                scriptSetupRanges.expose.define.start,
        ...
        for (const { define } of scriptSetupRanges.cssModules) {
            setupCodeModifies.push([
                ...
                define.arg ? [
                    ...
                    (0, utils_1.generateSfcBlockSection)(scriptSetup, define.arg.start, define.arg.end, index_1.codeFeatures.all),
                    `])`
                ] : [
        ...
        if (isTs) {
            setupCodeModifies.push([
                [
                    ...
                    (0, utils_1.generateSfcBlockSection)(scriptSetup, define.arg.start, define.arg.end, index_1.codeFeatures.navigation),
    ...
    if (scriptSetupRanges.props.define?.typeArg && scriptSetupRanges.props.withDefaults?.arg) {
        ...
        yield (0, utils_1.generateSfcBlockSection)(scriptSetup, scriptSetupRanges.props.withDefaults.arg.start, scriptSetupRanges.props.withDefaults.arg.end, index_1.codeFeatures.navigation);
...
function* generateDefineWithType(scriptSetup, name, define, expression, defaultName, typeName) {
    ...
    if (!name) {
        if (statement.start === expression.start && statement.end === expression.end) {
            ...
            yield [[
                    `const ${defaultName} = `,
                    (0, utils_1.generateSfcBlockSection)(scriptSetup, expression.start, typeArg.start, index_1.codeFeatures.all)
                ], statement.start, typeArg.start];
            yield [[
                    (0, utils_1.generateSfcBlockSection)(scriptSetup, typeArg.end, expression.end, index_1.codeFeatures.all),
                    utils_1.endOfLine,
                    (0, utils_1.generateSfcBlockSection)(scriptSetup, statement.start, expression.start, index_1.codeFeatures.all),
                    defaultName
            ...
            yield [[
                    `const ${defaultName} = `,
                    (0, utils_1.generateSfcBlockSection)(scriptSetup, expression.start, expression.end, index_1.codeFeatures.all),
                    utils_1.endOfLine,
                    (0, utils_1.generateSfcBlockSection)(scriptSetup, statement.start, expression.start, index_1.codeFeatures.all),
                    defaultName
...
function* generateComponentProps(options, ctx, scriptSetup, scriptSetupRanges, definePropMirrors) {
    ...
    if (scriptSetupRanges.props.define?.arg) {
        yield `props: `;
        yield (0, utils_1.generateSfcBlockSection)(scriptSetup, scriptSetupRanges.props.define.arg.start, scriptSetupRanges.props.define.arg.end, index_1.codeFeatures.navigation);
        yield `,${utils_1.newLine}`;
    }
    ...
    if (scriptSetupRanges.defineProp.length) {
        ...
        for (const defineProp of scriptSetupRanges.defineProp) {
            ...
            if (defineProp.modifierType) {
                ...
                yield `${propModifierName}?: Partial<Record<${modifierType}, true>>,${utils_1.endOfLine}`;
            }
        }
        yield `}`;
    }
